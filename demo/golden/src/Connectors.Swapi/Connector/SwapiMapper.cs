...
namespace Connectors.Swapi
{
    public static class SwapiMapper
    {
        ...
        private static readonly Regex GalacticYear = new(@"^(?<value>\d+(\.\d+)?)(?<era>BBY|ABY)$", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant);
        ...
        public static Product.Canonical.Character ToCharacter(Connectors.Swapi.Generated.Person source)
        {
            ...
            return new Product.Canonical.Character
            {
                ...
                // HUMAN DECISION: unparsable non-"unknown" values -> null; bad calendar data never fails a lookup.
                BirthYear = ParseGalacticYear(source.Birth_year),
                // HUMAN DECISION: "n/a" (droids) -> Other; only "unknown"/unrecognised -> Unknown.
                Gender = ParseGender(source.Gender),
                HairColors = SplitList(source.Hair_color),
        ...
        private static double? ParseGalacticYear(string? value)
        {
            if (IsSentinel(value)) return null;
            var m = GalacticYear.Match(value!.Trim());
            if (!m.Success) return null;
            var years = double.Parse(m.Groups["value"].Value, CultureInfo.InvariantCulture);
            return m.Groups["era"].Value.Equals("BBY", StringComparison.OrdinalIgnoreCase) ? -years : years;
        }

        private static Product.Canonical.Gender ParseGender(string? value) => value?.Trim().ToLowerInvariant() switch
        {
            "male" => Product.Canonical.Gender.Male,
            "female" => Product.Canonical.Gender.Female,
            "hermaphrodite" => Product.Canonical.Gender.Other,
            "n/a" => Product.Canonical.Gender.Other,
            _ => Product.Canonical.Gender.Unknown,
        };
