...
namespace Connectors.Swapi.Tests.Connector
{
    public class SwapiMapperTests
    {
        ...
        public void ToCharacter_maps_the_recorded_fixture()
        {
            ...
            Assert.Equal("1", result.ExternalId);                       // url -> last path segment
            Assert.Equal("swapi", result.Provider);                     // constant
            Assert.Equal("Luke Skywalker", result.Name);                // name
            Assert.Equal(172, result.HeightCm);                         // "172" -> 172
            Assert.Equal(77d, result.MassKg);                           // "77" -> 77
            Assert.Equal(-19d, result.BirthYear);                       // "19BBY" -> -19
            Assert.Equal(Gender.Male, result.Gender);                   // "male" -> Male
            Assert.Equal(new[] { "blond" }, result.HairColors);         // "blond" -> ["blond"]
            Assert.Equal("blue", result.EyeColor);                      // eye_color
            Assert.Equal("1", result.HomeworldId);                      // homeworld url -> "1"
            Assert.Equal(new[] { "1", "2", "3", "6" }, result.FilmIds); // films[] -> ids, order preserved
            Assert.Equal(source.Url, result.SourceUrl);                 // url -> sourceUrl
            Assert.Equal(source.Edited!.Value.ToUniversalTime(), result.LastModifiedUtc); // edited -> lastModifiedUtc
        }

        [Fact]
        public void ToCharacter_mass_strips_thousands_separator()
        {
            var jabba = Fixture<Person>("Person.jabba.json"); // mass "1,358"
            Assert.Equal(1358d, SwapiMapper.ToCharacter(jabba).MassKg);
            Assert.Equal(Gender.Other, SwapiMapper.ToCharacter(jabba).Gender); // hermaphrodite -> Other
            Assert.Equal(-600d, SwapiMapper.ToCharacter(jabba).BirthYear);
        }

        [Fact]
        public void ToCharacter_honours_decision_droids_are_Other()
        {
            var c3po = Fixture<Person>("Person.c3po.json"); // gender "n/a"
            Assert.Equal(Gender.Other, SwapiMapper.ToCharacter(c3po).Gender);
            Assert.Empty(SwapiMapper.ToCharacter(c3po).HairColors);         // "n/a" -> []
        }

        [Theory]
        [InlineData("unknown", null)]
        [InlineData("", null)]
        [InlineData("12BBY?", null)]      // HUMAN DECISION: unparsable -> null, never throw
        [InlineData("41.9BBY", -41.9)]
        [InlineData("5ABY", 5d)]
        [InlineData("0BBY", 0d)]
        public void ToCharacter_birth_year_follows_the_galactic_calendar_rule(string raw, double? expected)
        {
            var result = SwapiMapper.ToCharacter(Person(p => p.Birth_year = raw));
            Assert.Equal(expected, result.BirthYear);
        }

        [Theory]
        [InlineData("male", Gender.Male)]
        [InlineData("FEMALE", Gender.Female)]
        [InlineData("hermaphrodite", Gender.Other)]
        [InlineData("n/a", Gender.Other)]        // HUMAN DECISION
        [InlineData("unknown", Gender.Unknown)]
        [InlineData("wookiee", Gender.Unknown)]  // unrecognised -> Unknown
        [InlineData(null, Gender.Unknown)]
        public void ToCharacter_gender_vocabulary_is_explicit(string? raw, Gender expected)
        {
            Assert.Equal(expected, SwapiMapper.ToCharacter(Person(p => p.Gender = raw)).Gender);
        }

        [Fact]
        public void ToCharacter_handles_provider_sentinels()
        {
            var result = SwapiMapper.ToCharacter(Person(p =>
            {
                p.Height = "unknown"; p.Mass = "n/a"; p.Hair_color = "none"; p.Eye_color = "unknown";
                p.Homeworld = null; p.Films = null; p.Edited = null;
            }));
            ...
            Assert.Null(result.LastModifiedUtc);
        }

        [Fact]
        public void ToCharacter_splits_multi_valued_hair_colors()
        {
            var result = SwapiMapper.ToCharacter(Person(p => p.Hair_color = "brown, grey"));
            Assert.Equal(new[] { "brown", "grey" }, result.HairColors);
        }
